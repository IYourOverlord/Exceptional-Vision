...
### Багфикс 15 — Гонка потоков в `GeometryChangeDeduplicator`, `ArrayIndexOutOfBoundsException` при мешинге (2026-08-20)

**Симптом**: после Багфикса 14 (jarJar) секции стали реально доходить до мешинга (ранее
тихо отбрасывались на `isEmpty()` в `MeshWorkerPool.processTask`, до этого фикса эта ветка
кода не выполнялась вовсе), и на реальном прогоне игры сразу же посыпался краш воркер-потоков:

```
[ev-mesh-worker/WARN] Meshing failed for section 0@[57, -2, -3]
java.lang.ArrayIndexOutOfBoundsException: Index 8192 out of bounds for length 4097
	at dev.ev.render.dirty.GeometryChangeDeduplicator.recordAndCheckChanged(GeometryChangeDeduplicator.java:72)
	at dev.ev.render.scheduling.MeshWorkerPool.processTask(MeshWorkerPool.java:117)
```

**Причина**: `GeometryChangeDeduplicator` создаётся один раз в `EVInstance` и передаётся
единым инстансом в `MeshWorkerPool`, который запускает 8 воркер-потоков (`ev-mesh-worker`),
каждый из которых вызывает `recordAndCheckChanged` на этом общем инстансе без какой-либо
внешней синхронизации. Внутреннее хранилище — `it.unimi.dsi.fastutil.longs.
Long2LongOpenHashMap`, не потокобезопасный класс: конкурентные `put()` из разных потоков
могут одновременно триггерить внутренний resize/rehash backing-массива, из-за чего один
поток читает/пишет по индексу, посчитанному под старую ёмкость массива, уже увеличенного
другим потоком — отсюда `ArrayIndexOutOfBoundsException` с индексом, который явно больше
текущей длины массива (`8192` при `length 4097`, то есть ровно "следующая степень двойки минус
один" после уже свершившегося где-то параллельно ресайза). Дополнительно сама логика метода
(`get`, затем условный `put`) — это check-then-act, требующий атомарности даже при
потокобезопасной по себе структуре данных: две секции с одинаковым закодированным
`SectionPos`, обработанные конкурентно (переобсчёт одной и той же секции двумя воркерами
подряд — штатная ситуация в mesh-очереди), иначе могли бы гонять за тем, чей `newHash`
"победит" в мапе.

Баг не проявлялся ни в одном из тестов Багфиксов 11-14 и ни разу до этого сеанса, потому что
до фикса секций, реально доходящих до `recordAndCheckChanged`, не было вовсе (все task'и
отбрасывались раньше, на `handle.isEmpty()`) — сам факт того, что баг проявился именно сейчас,
это подтверждение, что предыдущий фикс сработал, а не новая регрессия от него.

**Исправление**: `GeometryChangeDeduplicator.recordAndCheckChanged` помечен `synchronized`
(`ev-render/src/main/java/dev/ev/render/dirty/GeometryChangeDeduplicator.java`) — весь метод
целиком, а не только `put()`, чтобы устранить и гонку резайза внутри fastutil-мапы, и
check-then-act гонку `get`/`put` для одной и той же секции. `hashQuads()` синхронизации не
требует — работает только с локальным для вызывающего потока `List<Quad>`, не трогает
`lastHashesByEncodedPos`. Добавлен Javadoc на класс, объясняющий требование
потокобезопасности явно (единственный инстанс шарится между всеми воркерами пула), чтобы
следующая сессия не убрала `synchronized` как "лишний" при рефакторинге, не зная контекста.

**Альтернатива, которая не была выбрана**: замена `Long2LongOpenHashMap` на
`ConcurrentHashMap<Long, Long>`/`java.util.concurrent`-эквивалент решила бы только гонку
резайза, но не check-then-act гонку той же секции (`computeIfAbsent`/`merge` потребовали бы
отдельного продумывания семантики "изменилось/не изменилось"), и добавили бы boxing
Long-ключей/значений на каждый вызов на hot path мешинга — `synchronized` на некрупном,
редко удерживаемом надолго методе (один `get`+один `put` в худшем случае) проще и не хуже по
контенции при текущем размере пула (8 воркеров), учитывая что сам мешинг одной секции
(построение geometrии в `pipelineRunner.build`) на порядки дороже, чем время удержания этого
лока.

**Тесты**: добавлен `GeometryChangeDeduplicatorTest.testConcurrentAccessDoesNotThrowOrCorruptState`
— 8 потоков, по 500 секций на поток (частично уникальные ключи на поток, чтобы гонять
map growth/resize под конкурентными писателями; частично один и тот же небольшой набор
ключей на все потоки, чтобы явно гонять get/put race по одной и той же секции), проверка
отсутствия исключений. Регрессионный тест reproduce'ит форму бага (общий инстанс + много
потоков + много секций), не проверяет конкретное содержимое мапы после гонки (порядок
выполнения потоков недетерминирован по дизайну теста).

**Не проверено фактическим запуском/сборкой в этой сессии** — как и во всех предыдущих
багфиксах, песочница не имеет доступа к Gradle/JDK для `./gradlew build`/`test`. Требует
подтверждения пользователем реальной пересборкой и повторным запуском той же сцены, где
воспроизводился краш (секция `0@[57, -2, -3]` и её окрестности).

`PROJECT_INDEX.md` обновлён — в "Ключевых классах" `ev-render` (`GeometryChangeDeduplicator`)
добавлено упоминание `synchronized` и причины, счётчик тестов `GeometryChangeDeduplicatorTest`
исправлен с 4 на 5 сценариев. Расхождений между разделами "Модульная карта"/"Статус тикетов"/
"Чего пока не существует" для этого класса при сверке не найдено — только "Ключевые классы"
были неактуальны по отношению к этому изменению.

### Багфикс 16 — Отсутствующий VAO перед `glMultiDrawArraysIndirect`, `GL_INVALID_OPERATION` каждый кадр (2026-08-20)

**Симптом**: экран пустой (ничего не рисуется), в GL debug-выводе каждый кадр (~17-20мс
интервал, то есть буквально на каждом draw-вызове) повторяется:

```
GL_INVALID_OPERATION (0x502) — 'Array object is not active.'
```

в `EVInstance frame cleanup`. Снапшот метрик пуст (queue depths/cache hit rates/counters —
`(none)`), что изначально выглядело как вторая, отдельная проблема (ничего не поставлено
в очередь мешинга), но по факту — не связано с этим багом: `MetricsRegistry` в MVP-пути
просто ни к чему не подключён (отдельный, более мелкий не закрытый вопрос, не текущий фикс).

**Причина**: `GLGraphicsPipeline.drawIndirect()`/`drawIndirectCount()`
(`ev-gpu/src/main/java/dev/ev/gpu/gl/GLGraphicsPipeline.java`) вызывают
`glMultiDrawArraysIndirect`/`glMultiDrawArraysIndirectCountARB` без предварительного
`glBindVertexArray()`. В Core Profile OpenGL 4.5 (используется здесь) любой draw-вызов
требует активного (забинженного) VAO — даже пустого, без единого атрибута — если геометрия
целиком генерируется процедурно в вершинном шейдере из `gl_VertexID`/`gl_InstanceID`
(vertex-pulling), как и задумано в `FarLodPassRenderer` (комментарий класса явно говорит
"vertex-pulling, no VAO/VBO"). Этот комментарий верен насчёт отсутствия необходимости в VBO
с реальными вершинными данными, но ошибочен насчёт VAO — спецификация Core Profile требует
забинженный VAO безусловно, независимо от того, есть ли в нём атрибуты. Без него GL считает
"массив не активен" и не рисует ничего, отсюда `GL_INVALID_OPERATION` на каждом кадре и
пустой экран.

**Исправление**: в `GLGraphicsPipeline` заведён один пустой (без атрибутов) `dummyVao` —
создаётся один раз в конструкторе (`glCreateVertexArrays()`), биндится
(`glBindVertexArray(dummyVao)`) перед обоими draw-вызовами (`drawIndirect`,
`drawIndirectCount`), удаляется в `free()` вместе с программой, чтобы не течь GL-хендлами
при пересоздании пайплайна (например, при hot-reload шейдеров).

**Подтверждение реальным прогоном**: пользователь пересобрал и запустил — `GL_INVALID_OPERATION`
и "Array object is not active" в логах/debug-выводе больше не воспроизводятся, отрисовка
происходит без краша каждый кадр (см. приложенный скриншот и `latest.log`/`debug.log` сессии
от 2026-08-20 20:27-20:30 — ни одной строки `dev.ev`-происхождения с уровнем `ERROR`, только
штатные `INFO` про инициализацию `EVInstance`/`MeshWorkerPool`). Это первый багфикс в этом
журнале, подтверждённый не текстовым разбором, а фактическим запуском игры пользователем.

**Отдельно, не баг — ожидаемое поведение MVP-заглушки**: на скриншоте после фикса видны
однотонные цветные (маджента/зелёный/серый) полосы вместо текстурированных вокселей. Это не
новая проблема и не регрессия от VAO-фикса — `FarLodPassRenderer` в текущем виде
(`ev-neoforge/src/main/java/dev/ev/neoforge/FarLodPassRenderer.java`) намеренно рисует
debug-цветные unit-кубы по bounding sphere каждой видимой секции (`vColor = vec3(hue, 1-hue,
0.5)` от `gl_InstanceID % 8`, см. Javadoc класса: "no real meshlet/quad geometry from the
meshing pipeline is consumed here yet, only each visible section's bounding box"). Реальный
меш из `ev-meshing`/`ev-gpu` в этот draw-путь пока не подключён — замена заглушки на
настоящую meshlet-геометрию не входит в тикеты Волны 1 в их текущей форме и является
последующей работой, а не дефектом этого фикса.

**Не найдено расхождений** в `PROJECT_INDEX.md` для `ev-gpu` при сверке (класс
`GLGraphicsPipeline` уже был описан в "Ключевых классах"; добавлено упоминание `dummyVao`
и причины его необходимости, без изменения статуса тикета — 17/18 остаются `DONE`, это
багфикс внутри уже реализованного класса, а не новый функционал).

### Багфикс 17 — `runClient` classpath не включал sibling-подмодули, `NoClassDefFoundError`
на `MetricsRegistry` при входе в мир (2026-08-20)

**Симптом**: после фикса Багфикса 16 пользователь попытался выполнить
`P0-profiling-checkpoint.md` (запустил `./gradlew runClient` с добавленным JFR
`jvmArgument`, как описано в `jfr-profiling-runclient-snippet.md`). Игра запускалась,
но краш происходил сразу при создании мира (`ClientLevel.<init>` → `EV.onLevelLoad`):
`java.lang.NoClassDefFoundError: dev/ev/api/metrics/MetricsRegistry`, `Caused by:
ClassNotFoundException`.

**Диагностика**: `NoClassDefFoundError` (не `ClassNotFoundException` напрямую) означает,
что класс существовал на этапе компиляции (`ev-neoforge` иначе не собрался бы), но
отсутствует в рантайм-classpath. `debug`-лог `runClient` (`Got mod coordinates ev%%...`)
подтвердил: в classpath рана передавался **только** `ev-neoforge/build/classes/java/main`
— ни `ev-api`, ни `ev-storage`/`ev-meshing`/`ev-gpu`/`ev-render` там не было вообще.
Проверка `./gradlew build -x test -x check` показала, что все модули компилируются без
ошибок (включая `ev-api:compileJava`) — то есть проблема не в компиляции и не в устаревшем
кэше (`--rerun-tasks` не помог), а в самой конфигурации `runClient`.

**Причина**: `ev-neoforge/build.gradle.kts` уже содержал `jarJar(project(":ev-api"))` и
т.д. (закрывает embedding для финального дистрибутивного jar), а также обычные
`implementation(project(":..."))` (закрывает compile-classpath). Но ModDevGradle 2.0.141
не выводит classpath именно для **dev-запуска** (`runClient`) автоматически из обычных
Gradle project-зависимостей в multi-module setup — он берёт classpath рана только из
sourceSet'ов, явно зарегистрированных в блоке `neoForge { mods { create(...) {
sourceSet(...) } } }`. В блоке был зарегистрирован только `sourceSets.main.get()` самого
`ev-neoforge` — ни один из sibling-подмодулей туда не попадал.

**Исправление**: в `ev-neoforge/build.gradle.kts`, блок `mods { create(mod_id) { ... } }`
дополнен явной регистрацией `sourceSet(...)` для main source set каждого из пяти
подмодулей (`ev-api`, `ev-storage`, `ev-meshing`, `ev-gpu`, `ev-render`) через
`project(":...").extensions.getByType<SourceSetContainer>()["main"]`, в дополнение к уже
существующей регистрации `sourceSets.main.get()` самого `ev-neoforge`. `jarJar`/
`implementation` записи в `dependencies {}` оставлены без изменений — они закрывают
другие, независимые задачи (embedding в дистрибутивный jar и compile-classpath
соответственно), не dev-run classpath.

**Подтверждение реальным прогоном**: пользователь пересобрал (`--rerun-tasks`) и запустил
— `NoClassDefFoundError` больше не воспроизводится. Лог показывает полную
последовательность: `Client level loaded, initializing EVInstance` →
`Started 8 EV meshing worker threads` → `EV client commands registered`, без единой
ERROR-строки происхождения `dev.ev`. Пользователь провёл в мире ~38 секунд, корректно
вышел (`Client level unloaded, shutting down EVInstance` → `EVInstance closed`).

**Изменённые файлы**: только `ev-neoforge/build.gradle.kts` (один добавленный блок
регистрации sourceSet, без удалений/изменений прочих частей файла).

**Не найдено расхождений** в `PROJECT_INDEX.md`, требующих исправления в рамках этого
багфикса — правка ограничена build-конфигурацией, не затрагивает классы/модульную карту.

### P0-profiling-checkpoint — [/] в процессе, первый частичный прогон (2026-08-20)

После Багфикса 17 пользователь смог зайти в мир и предоставил `ev-coldstart.jfr`
(запись `settings=profile`, ~62.3 сек wall-clock, сессия 22:16:20–22:17:21) вместе со
скриншотом и `latest.log` того же прогона.

**Что сделано**: JFR-файл проанализирован напрямую через `jdk.jfr.consumer` API (JMC не
был доступен в среде анализа — CLI `jfr` отсутствовал в установленном JDK). Извлечены:
разбивка `jdk.ExecutionSample` (5643 событий) по top-of-stack классам/потокам,
`jdk.JavaMonitorEnter` события (lock contention) с фильтром по `SectionCache`/
`MeshTaskQueue`-related классам. Результаты записаны в `PROFILING_RESULTS.md` (создан в
корне репозитория, ранее отсутствовал в git — существовал только как черновик вне репо).

**Ключевые находки**: vanilla world generation доминирует в CPU-сэмплах (56.3%) —
ожидаемо, так как тестовый мир был свежесгенерированным без прегенерации, что прямо
нарушает Требование к реализации п.1 тикета ("большой мир с разнообразным рельефом").
EV-код — 7.7% CPU-сэмплов. Внутри видимого EV meshing-потока (`ev-mesh-worker`, 127
сэмплов) доминирует `GreedyMeshStage.meshFaceDirection` (65.4%). Lock contention:
0 событий, привязанных к `SectionCache`/`MeshTaskQueue` — единственные два тикета из
таблицы решений, для которых теперь есть измеренное (не гаданное) решение: `08-opt` и
`15-opt` переведены с "неизвестно/отложить" на "пропустить (по этим данным)".

**Явное отклонение от полноты, требуемой тикетом**: критерий приёмки п.1
("PROFILING_RESULTS.md создан... с реальными числами по **обоим** сценариям") выполнен
только частично. Сценарий A закрыт лишь частично (throughput/разбивка по стадиям —
TODO, нет `MetricsRegistry`/`ImportStageStatus` источника в этом JFR; Time-to-first-frame/
Time-to-visually-complete — TODO, не измерялись секундомером/видеозаписью). **Сценарий B
не закрыт вообще** — ни одна метрика (frame time, CPU/GPU breakdown, draw calls,
overdraw) не была измерена в этом прогоне; GPU-сторона в принципе не покрывается CPU-JFR
сэмплированием (LWJGL/GL — 0.1% сэмплов), нужен RenderDoc или `/ev profile` (тикет 29),
ни один не использовался. 8 из 10 opt-тикетов в итоговой таблице остаются "отложить" по
причине реального отсутствия данных, не потому что они не нужны.

**Чекбокс в разделе "Статус выполнения задач" НЕ отмечен `[x]`** — оставлен `[ ]` (не
начато) в начале файла, так как тикет прямо требует все 10 строк с содержательным
решением (не только "отложить" из-за отсутствия данных) и оба сценария измеренными;
текущее состояние — только первый частичный диагностический срез, не завершение тикета.
`PROJECT_INDEX.md` не обновлялся отдельной секцией "Решения по opt-тикетам" в рамках этой
записи — критерий приёмки п.4 тикета явно требует эту секцию только когда решения по
opt-тикетам приняты по существу ("внедрять"/"пропустить" на основе полных данных), а не
на промежуточном этапе с преимущественно "отложить"; это стоит сделать после следующего,
более полного прогона, а не сейчас, чтобы не создавать в `PROJECT_INDEX.md` запись,
которую тут же придётся переписывать.

**Следующий шаг** (не выполнен в этой сессии, оставлен пользователю): повторный
профилирующий прогон на прегенерированном/уже посещённом большом мире, дольше по
времени, с `/ev profile` или RenderDoc для GPU-стороны и Сценария B целиком — см. раздел
"Следующие шаги для полного закрытия чекпоинта" в `PROFILING_RESULTS.md`.

### Находка (не багфикс, код не тронут) — `DirtySectionTracker` никогда не наполняется,
EV-конвейер простаивает вхолостую (2026-08-20)

**Контекст обнаружения**: пользователь выполнил более длительный забег с `/tp` на большие
координаты (2000/2000, затем 2000/20000) для повторного прогона `P0-profiling-checkpoint`
и заметил визуальный баг — дальность видимой геометрии обрывается через несколько блоков.
Приложенные `latest.log` и скриншот проанализированы; `ev-coldstart.jfr` этого прогона
получен, но ещё не проанализирован отдельно от находки ниже.

**Находка**: три вызова `/ev debug` в течение прогона (23:16:37, 23:17:27, 23:18:23 — в
том числе сразу после телепорта на 20000 по Z) все показывают идентичный пустой снапшот:
```
Import Progress: 0/0 (0.0%)
  queued: 0  retrying: 0  building: 0  done: 0
Queue Depths: (none)
Cache Hit Rates: (none)
GPU Pass Durations: (none)
Counters: (none)
```
Ни одна секция ни разу не была поставлена в очередь на мешинг за весь ~5-минутный прогон,
несмотря на телепортацию в непосещённые области. Проверка кода (`grep -rn "markDirty"`
по всему `ev-neoforge`/`ev-render`, вне тестов) показала: **`DirtySectionTracker.
markSectionDirty(...)` не вызывается НИГДЕ в production-коде** — ни в `EV.java`, ни в
`EVInstance.java`, ни где-либо ещё за пределами `ev-api` (где метод объявлен только как
контракт интерфейса) и тестовых фейков.

**Причинно-следственная цепочка**: `DirtySectionTracker` пуст с самого начала →
`MeshSchedulingCoordinator.drainDirtyAndSchedule()` (вызывается каждый кадр из
`EVInstance.renderFarLod`) видит `dirtyTracker.hasPendingWork() == false` и сразу
выходит, ничего не планируя → `MeshTaskQueue` никогда не получает задач → 8 запущенных
worker-потоков (`MeshWorkerPool`, факт старта подтверждён логом) простаивают всю сессию
→ `geometryMap.loadedSections()` остаётся пустым → `SimpleTraversal.computeVisible(...)`
возвращает пустой список → `FarLodPassRenderer` не рисует ни одной far-LOD секции.
**Видимая на скриншоте геометрия — чистый ванильный Minecraft** (view distance был
увеличен в сессии до 25 чанков ≈ 400 блоков), без единого вклада EV поверх неё; "обрезка
дальности радиусом в несколько блоков" — это край обычной ванильной прогрузки чанков, не
EV-специфичный culling-баг, как изначально показалось при осмотре скриншота.

**Сверка с текстом тикетов**: `tickets/31-integration-checklist-mvp.md`, Шаг 4, явно
требует обработчик block-change NeoForge-события, вызывающий `DirtySectionTracker.
markSectionDirty` — этого обработчика в коде тоже не найдено (тот же `grep` для
`markDirty`/`markSectionDirty` пуст вне `ev-api`/тестов). Но даже полная реализация этого
конкретного требования Шага 4 **не решила бы** проблему полностью: block-change событие
реагирует только на правки блоков игроком, не на первичную генерацию/загрузку новой
территории при обычном перемещении — а именно это нужно, чтобы far-LOD секции появлялись
при заходе в непосещённые области без ручной правки блоков. Ни `tickets/31-integration-
checklist-mvp.md`, ни `tickets/26-render-dirty-tracking-mvp.md` не специфицируют явно
источник первичного (initial/seed) наполнения `DirtySectionTracker` при загрузке чанка —
это пробел в тексте самих тикетов, не только пропуск при их реализации.

**Действие в этой сессии**: только диагностика и документирование, код НЕ изменён.
Причина — находка требует архитектурного решения (какое именно NeoForge-событие
использовать как триггер: `ChunkEvent.Load`, `RenderLevelStageEvent` с проверкой позиции
игрока по сетке секций, или что-то ещё — не очевидно без отдельного исследования/
уточнения через web search точного API события, как и предполагает сам стиль тикетов
31/27), и это осознанно оставлено как явный выбор пользователя — чинить сейчас в этой же
сессии или оформить отдельным тикетом/задачей.

**Влияние на текущий профилирующий чекпоинт**: `PROFILING_RESULTS.md` (записан ранее в
этой сессии) косвенно затронут этой находкой — та часть анализа `ev-coldstart.jfr`
(первого прогона), где EV-код показал только 7.7% CPU-сэмплов с доминированием vanilla
worldgen, теперь получает объяснение: EV-конвейер не только был заглушён генерацией
нового мира (как отмечено тогда), но и в принципе не активировался бы даже на уже
прогруженном мире, потому что нет источника dirty-меток. Второй JFR-прогон
(`ev-coldstart.jfr` из этой находки) пока не проанализирован отдельно — с учётом этой
находки его анализ, вероятно, покажет то же самое (EV простаивает) и не даст новой
информации о производительности EV-подсистем сверх уже известного факта простоя; анализ
этого второго JFR явно отложен до решения по этой находке, чтобы не тратить время на
профилирование неактивного конвейера.

**PROJECT_INDEX.md не обновлялся** в рамках этой записи — находка описывает поведенческий
пробел в интеграции (отсутствующий вызов), не расхождение состояния модульной карты/
статуса тикетов с деревом файлов; сверка (`find`/`view` по `ev-render`/`ev-neoforge`)
не проводилась отдельно для этой находки, так как сам факт находки уже получен через
прямой `grep`, не требующий сверки таблиц `PROJECT_INDEX.md`.


### Обнаруженный разрыв журнала (не заполняется задним числом) — между "Находкой про DirtySectionTracker" и текущей сессией (найдено 2026-08-21)

При старте этой сессии сверка фактического дерева файлов с последней записью журнала (находка
"`DirtySectionTracker` никогда не наполняется", код не тронут) показала расхождение: в
`ev-neoforge/src/main/java/dev/ev/neoforge/EV.java` уже присутствует полностью реализованный
обработчик `onChunkLoad(ChunkEvent.Load)`, вызывающий `voxelizeSection(...)` и
`instance.dirtyTracker().markSectionDirty(section)` для каждой level-0 секции чанка — то есть
именно тот фикс, который предыдущая запись журнала описывала как "требует архитектурного решения,
оставлено пользователю". Этот фикс явно был выполнен в отдельной сессии между той находкой и
текущей, но соответствующая запись в этом журнале отсутствует.

**Это не восполняется задним числом в рамках текущей сессии** — по правилу append-only журнала
(см. `EV_SESSION_PROMPT.md`, раздел "PROGRESS.md"), реконструкция чужой сессии по содержимому
кода без доступа к её реальному ходу работы (какие альтернативы рассматривались, почему выбран
именно `ChunkEvent.Load`, а не `RenderLevelStageEvent`-based опрос позиции игрока, упомянутый как
альтернатива в самой находке) была бы недостоверной записью, выдаваемой за исторический факт.
Решение о ретроактивном заполнении (или подтверждении, что фикс достаточно самоочевиден по коду
и заполнение не нужно) оставлено пользователю — см. также требование `EV_SESSION_PROMPT.md`
явно называть подобные находки, а не дописывать пробел молча.

Задача текущей сессии (см. запись ниже) начата с того состояния кода, в котором этот фикс уже
присутствует — он не является частью выполненной здесь работы, только зафиксирован как
контекст, почему `EV.java` в этой сессии выглядит не так, как последняя запись журнала могла бы
предполагать.

### Production MetricsRegistry — замена `NoopMetricsRegistry` в `EVInstance` (2026-08-21)

**Контекст задачи**: по итогам `P0-profiling-checkpoint` пользователь сообщил о двух независимых
находках в одном прогоне (23:14:57–23:16:29): (1) EV-конвейер реально работает —
`onChunkLoad`/`voxelizeSection` занимают 77.6% времени main-потока, `ev-mesh-worker` проходит
всю цепочку стадий мешинга устойчиво на протяжении записи, не разово, что подтверждает фикс
`ChunkEvent.Load` из необнаруженной в этом журнале предыдущей сессии (см. запись выше) реальными
данными, не только чтением кода; (2) `/ev debug` тем не менее пуст по отдельной, независимой
причине — `EVInstance` инстанцировал приватный inline `NoopMetricsRegistry` (все методы —
пустые тела) как единственную production-реализацию `MetricsRegistry`, поэтому `snapshot()`
гарантированно пуст независимо от того, работает ли конвейер. Задача этой сессии — реализовать
production `MetricsRegistry` и заменить `NoopMetricsRegistry` в `EVInstance`.

**Сверка перед началом**: `grep -rn "MetricsRegistry" --include=*.java ev-neoforge ev-render
ev-api | grep -v "/test/"` подтвердил формулировку пользователя — единственная production-точка
инстанцирования была `this.metricsRegistry = new NoopMetricsRegistry();` в конструкторе
`EVInstance` (приватный inner-класс в том же файле, все методы интерфейса — no-op тела,
`snapshot()` возвращает `MetricsSnapshot.empty()`). Отдельные package-private
`NoopMetricsRegistry` в `ev-storage/src/test`/`ev-meshing/src/test` — независимые тестовые
фейки в других модулях/пакетах, не тот же класс, не тронуты.

**Реализация**: новый класс `dev.ev.neoforge.metrics.DefaultMetricsRegistry`
(`ev-neoforge/src/main/java/dev/ev/neoforge/metrics/DefaultMetricsRegistry.java`). Контракт
интерфейса (`MetricsRegistry` Javadoc) требует потокобезопасности и отсутствия аллокаций на hot
path в общем случае — вызывающие стороны подтверждены прямым `grep` по всем `main`-source set'ам:
`SectionCache.recordCacheAccess` (`ev-storage`, на каждом обращении к кэшу секций, с любого
worker-потока), `MeshTaskQueue.recordQueueDepth` (`ev-meshing`, на каждой постановке в очередь),
`FrameGraphBuilder.recordGpuPassDuration`/`SimpleTraversal.recordGpuPassDuration` (`ev-render`,
раз за проход/кадр на render-потоке), `CoarseSectionGenerator.recordCounter`
(`ev-storage`). Выбор структур данных под каждый метод:
- `recordQueueDepth`/`recordCounter` — `ConcurrentHashMap<String, AtomicInteger/LongAdder>`,
  запись напрямую в уже существующий элемент через `computeIfAbsent` (аллокация происходит
  только на первый вызов с новым именем ключа, не на каждый вызов).
- `recordCacheAccess` — пара `LongAdder` hits/total на имя кэша (не одна составная структура) —
  `LongAdder` выбран вместо `AtomicLong` намеренно: это самый частый call site (каждый lookup
  `SectionCache` с любого worker-потока), `LongAdder` разменивает чуть более дорогое чтение на
  меньшую contention под конкурентной записью, что здесь уместнее.
- `recordGpuPassDuration` — plain volatile `long` (не `AtomicLong`/`AtomicReference`) на имя
  прохода: семантика "последнее значение" (текущая длительность прохода), не аккумулятор — не
  нужен read-modify-write, только get/set.
- `recordImportStageStatus` — единственный `AtomicReference<ImportStageStatus>`, заменяется
  целиком; интерфейс уже документирует низкую ожидаемую частоту вызова (например, раз в тик), 
  дополнительное батчирование не добавлялось.
- `snapshot()` — единственный аллоцирующий метод: копирует все карты в отсортированные
  (`TreeMap` → `Map.copyOf`) неизменяемые `Map`, для детерминированного вывода
  `MetricsSnapshotFormatter` (который и так сортирует по ключу перед печатью — `TreeMap` здесь
  просто заранее приводит к тому же порядку, не меняет наблюдаемое поведение форматтера).
  Намеренно не координируется глобально против конкурентных `recordXxx` вызовов сверх гарантий,
  которые уже даёт `ConcurrentHashMap`/`LongAdder`/volatile сами по себе — снапшот может
  захватить разные метрики на слегка разные моменты времени; это приемлемо для diagnostic-вида
  (`/ev debug`, вызывается игроком) и на порядок дешевле, чем координация полностью консистентного
  снапшота по всем метрикам сразу.

`EVInstance.java`: импорт `NoopMetricsRegistry` (private inner class) убран; конструктор изменён
с `new NoopMetricsRegistry()` на `new DefaultMetricsRegistry()`; сам inner-класс
`NoopMetricsRegistry` удалён из файла целиком (был недокументирован публично, использовался
только в этом единственном месте).

**Тесты**: `DefaultMetricsRegistryTest`
(`ev-neoforge/src/test/java/dev/ev/neoforge/metrics/DefaultMetricsRegistryTest.java`), 8
сценариев: пустой снапшот равен `MetricsSnapshot.empty()`; `recordQueueDepth` — gauge-семантика
(overwrite, не накопление); `recordCacheAccess` — корректный hit rate, независимый между разными
именами кэшей; `recordGpuPassDuration` — last-write-wins, не среднее/сумма;
`recordCounter` — накопление delta, независимое между именами счётчиков;
`recordImportStageStatus` — полная замена значения, и отдельно — `NullPointerException` на
`null` (интерфейс/Javadoc класса заявляют non-null контракт); конкурентный сценарий — 8 потоков
× 2000 операций одновременно на `recordCounter`/`recordCacheAccess`/`recordQueueDepth` (тот же
паттерн доступа, что и реальный `SectionCache`/`MeshTaskQueue` — много worker-потоков, один общий
инстанс registry), с точной (не приблизительной) проверкой суммы `LongAdder`-счётчика и
корректного hit rate 0.5 при равном split hit/miss между потоками — подтверждает отсутствие
потерянных инкрементов под конкуренцией.

**Не проверено фактическим запуском/сборкой в этой сессии** — как и во всех предыдущих
задачах, песочница не имеет доступа к Gradle/JDK (см. "Компиляция и тесты" в
`EV_SESSION_PROMPT.md`). Требует подтверждения `./gradlew :ev-neoforge:test` пользователем, а
затем — реального прогона в игре и повторного вызова `/ev debug` на той же прогруженной сцене,
где ранее снапшот был гарантированно пуст, чтобы подтвердить, что теперь он действительно
что-то показывает (queue depths/cache hit rates/counters), а не просто что тесты класса
проходят изолированно.

`PROJECT_INDEX.md` обновлён — все четыре раздела ("Статус тикетов" — запись тикета 29 дополнена
находкой, плюс новая session-level строка "production MetricsRegistry"; "Модульная карта" —
строка `ev-neoforge` дополнена новым пакетом `dev.ev.neoforge.metrics` и классом
`DefaultMetricsRegistry`; "Ключевые классы" — секция `ev-neoforge` дополнена описанием
`DefaultMetricsRegistry` и обновлённым описанием конструктора `EVInstance`; "Чего пока не
существует" — сверен, отдельного упоминания этого пробела там не было и по факту не требуется,
так как теперь это не открытый пробел) согласованы за один проход, без точечного добавления
только в одну таблицу.


### Stale far-LOD sections never evicted — "мерцающая стена наложенных текстур" после /kill (2026-08-21)

**Контекст задачи**: пользователь предоставил 2 скриншота (артефакт — стена наложенных
мерцающих текстур перед камерой, двигающаяся вместе с ней, перекрывающая обзор), `latest.log`,
`debug.log` и `ev-coldstart.jfr` из живого прогона. Хронология по логам: зашёл в существующий
(ранее посещённый) мир — изначально только ванильная прорисовка, `far-lod-pass: 0.00ms`,
`visible-sections: 0`; выполнил `/kill` → респавн на точке возрождения → артефакты появились
немедленно, следующий снапшот `/ev debug` показал `visible-sections: 162326`.

**Диагностика**: JFR-файл разобран через `jdk.jfr.consumer.RecordingFile` (single-file source
launch `java JfrDump.java`, `javac` в песочнице недоступен — как и в прошлых сессиях; см. также
класс не был найден изначально через `jfr` CLI, тоже отсутствует). Собрано и проверено все 494
события `jdk.JavaErrorThrow`/`jdk.JavaExceptionThrow` в записи — ни одно не содержит фрейма
`dev.ev` в стеке. Это исключило крэш/exception как причину и подтвердило архитектурную природу
проблемы. `jdk.ExecutionSample` с топ-фреймом в `dev.ev` показал `SectionGeometryMap#loadedSections`
на втором месте по частоте сэмплов после стадий мешинга — подозрительно много для геттера,
что стало отправной точкой для чтения `SectionGeometryMap.java`.

**Найдены две независимые причины, действовавшие вместе**:
1. `SectionGeometryMap.remove(SectionPos)` (`ev-render/.../scheduling/SectionGeometryMap.java`)
   был реализован полностью корректно с самого начала (Javadoc даже говорит "e.g. on unload"),
   но не имел ни одного вызывающего кода нигде в проекте — подтверждено `grep -rn "\.remove("`
   по всем `main`-source set'ам, единственное вхождение было само определение метода. Секции,
   попавшие в карту геометрии через `put()` при завершении мешинга, оставались резидентными
   навсегда, независимо от текущей позиции игрока.
2. `EVConfig.maxRenderDistanceBlocks` (дефолт 4096, `ev-neoforge/.../config/EVConfigLoader.java`)
   читается конфиг-лоадером и печатается в `/ev reload-config` (`EVCommands.java:167`), но
   `grep -n "maxRenderDistance" -r ev-neoforge/src/main` подтвердил — больше нигде не
   используется. `FrustumTester`/`SimpleTraversal` делают только геометрический
   sphere-vs-6-planes тест (`ev-render/.../culling/`), без какой-либо проверки дистанции
   от игрока. `EVInstance.calculateNearCutoffBlocks()` тоже объявлен и покрыт тестом
   (`EVInstanceTest.testCalculateNearCutoffBlocksCircumscribesSquare`), но `grep` подтвердил —
   нигде не вызывается за пределами этого теста.

Результат совпадения: после `/kill` секции, намешенные при исследовании мира на прошлой,
далёкой позиции игрока (никогда не выгружавшиеся), продолжали каждый кадр проходить через
frustum-тест. Frustum-тест по одному направлению/геометрии не знает о реальной дистанции —
если давняя секция геометрически пересекалась с новым view frustum по направлению (что при
достаточно широком угле обзора и произвольном положении почти неизбежно для какой-то части
из тысяч накопленных секций), она проходила `isVisible() == true` и рисовалась как гигантский
неуместный куб (EV MVP рисует каждую видимую секцию как unit cube — см. запись
"far-lod-pass draw" выше) — источник артефакта "мерцающая стена".

**Фикс, две взаимодополняющие части** (ни одна не достаточна сама по себе — см. обоснование
ниже):

**(a) `EV.onChunkUnload(ChunkEvent.Unload)`** (`ev-neoforge/.../EV.java`) — новый обработчик,
зарегистрирован на `NeoForge.EVENT_BUS` рядом с существующим `onChunkLoad`. Для каждого level-0
Y-среза выгружаемой чанк-колонки (`ChunkPos.getMinBlockX()/getMinBlockZ()`, цикл по Y с шагом
32 от `level.getMinBuildHeight()` до `getMaxBuildHeight()` — тот же паттерн, что и в
`onChunkLoad`/`voxelizeSection`, только зеркально на unload) вызывает
`instance.geometryMap().remove(section)`. Потребовался новый публичный геттер
`EVInstance.geometryMap()` (поле раньше не было доступно снаружи класса).

**(b) `EVInstance.filterByDistance(...)`** (новый package-private static метод) — фильтрует
`geometryMap.loadedSections()` по дистанции камера↔bounding-sphere каждой секции
(`sqrt(dx²+dy²+dz²) <= maxDistanceBlocks + радиус_секции`, радиус учтён так, чтобы большая
секция не отбрасывалась только из-за того, что её центр чуть-чуть за чертой, пока часть объёма
ещё в бюджете), вызывается в `renderFarLod` шагом 5 **перед** `SimpleTraversal.computeVisible` —
и сам `SimpleTraversal` избавлен от лишней работы над заведомо далёкими секциями, и класс
`SimpleTraversal` не пришлось трогать. Переиспользует существующий
`EVConfig.maxRenderDistanceBlocks()` как единственный источник правды по дальности far-LOD,
вместо того чтобы вводить отдельный параметр. Метод намеренно вынесен как чистая
static-функция без доступа к полям инстанса (принимает `sectionBounds` как
`Function<SectionPos, float[]>` параметром) — специально ради юнит-тестируемости без
GL-контекста/`Minecraft`-инстанса, тот же паттерн, которым уже пользуется
`calculateNearCutoffBlocks`.

**Почему нужны обе части**: (a) без (b) всё ещё оставляет секции резидентными и валидными для
отрисовки до момента, пока соответствующий чанк на старой позиции физически не выгрузится на
клиенте (может не произойти сразу после телепорта — например, если чанк всё ещё в радиусе
прогрузки другого игрока на сервере, или просто не успел выгрузиться к моменту следующего
кадра). (b) без (a) останавливает отрисовку немедленно (правильное поведение сразу после
`/kill`), но `SectionGeometryMap` продолжает расти неограниченно на длинных сессиях —
утечка памяти, а `loadedSections()` (уже фигурировавший как заметно "горячий" в
`ExecutionSample`) продолжает перебирать всё большее число заведомо нерелевантных секций
каждый кадр.

**Тесты**:
- `SectionGeometryMapTest` (`ev-render/src/test/.../scheduling/`, новый файл) — 4 сценария:
  базовая эвикция через `remove()`, эвикция только целевой секции при наличии соседей,
  безопасный no-op при `remove()` секции, которая никогда не была добавлена, эвикция всех
  4 Y-срезов одной колонки — тот самый паттерн, которым реально пользуется `EV.onChunkUnload`.
- `EVInstanceTest` — 4 новых сценария на `filterByDistance`: секция в бюджете сохраняется,
  далёкая отбрасывается; прямое воспроизведение сценария бага (секция на координатах,
  похожих на упомянутые в логе — ~(6200,0,62500) — отбрасывается, когда камера обратно у
  спawn'а после `/kill`); учёт радиуса секции на границе cutoff (центр чуть за чертой, но
  сфера ещё пересекает бюджет — секция сохраняется); сохранение относительного порядка и
  корректная обработка пустого входного списка.

**Не подтверждено реальным Gradle-билдом или повторным запуском в игре в этой сессии** —
песочница по-прежнему без доступа к Gradle/JDK-компилятору (`javac` отсутствует, только `java`
рантайм, что позволило проанализировать JFR через single-file source launch, но не собрать сам
мод). Требует от пользователя `./gradlew :ev-neoforge:test :ev-render:test`, а затем повторения
точного сценария бага в живой игре (заход в тот же мир → накопление секций при исследовании →
`/kill`) для визуального подтверждения, что артефакт больше не появляется.

**Отдельно замечено, не в scope этого фикса**: `metricsRegistry.recordCounter("visible-sections",
...)`/`recordCounter("loaded-sections", ...)` в `renderFarLod` вызываются каждый кадр, но оба
семантически являются gauge-значениями ("сколько сейчас"), а `recordCounter` в
`DefaultMetricsRegistry` аккумулирует через `LongAdder` — отсюда `visible-sections: 162326` в
логе пользователя, что заметно больше правдоподобного реального количества видимых секций за
один кадр. Сама по себе эта неточность не является причиной визуальных артефактов (влияет
только на достоверность чисел в `/ev debug`, не на то, что реально рисуется), поэтому не
исправлялась в рамках этой сессии — но стоит знать, что число `visible-sections` в логах
пользователя из этого репорта было завышено этим известным багом метрик, а не отражает
фактическое количество нарисованных секций.

`PROJECT_INDEX.md` обновлён — все четыре раздела согласованы: "Статус тикетов" (новая
session-level строка), "Модульная карта" (`ev-render` уже упоминал `dev.ev.render.scheduling`/
`SectionGeometryMap` в общих чертах из прошлой сессии, изменений не потребовалось), "Ключевые
классы" (`ev-render` — дополнена запись `SectionGeometryMap`, добавлен пакет
`dev.ev.render.scheduling` в заголовок раздела, который раньше не упоминался, хотя класс уже
существовал; `ev-neoforge` — дополнены записи `EV`/`EVInstance`/`DefaultMetricsRegistry`),
"Чего пока не существует" — сверен, отдельного пункта под эту находку не требуется, так как
теперь это не открытый пробел.

## Сессия: camera-relative coordinate mismatch в far-LOD рендеринге (visible-sections: 0 на больших координатах)

**Контекст**: пользователь сообщил о продолжении проблем с прорисовкой геометрии после
предыдущего фикса `filterByDistance`/`onChunkUnload`. Анализ debug-снапшотов и лога показал два
разных явления, требовавших разделения:

1. `visible-sections: 375252` (третий снапшот, после `/kill`) — при перепроверке оказался
   **не багом**, а известным, уже задокументированным в прошлой сессии артефактом накопительной
   семантики `recordCounter`/`LongAdder` в `DefaultMetricsRegistry` (счётчик суммируется по
   кадрам, а не отражает значение за один кадр). Реальное число видимых секций за кадр
   (перепроверено отдельно) — порядка нескольких сотен, правдоподобно для near-camera геометрии.
   Этот пункт не требовал нового фикса.
2. `visible-sections: 0` в первых двух снапшотах, при этом `meshes-uploaded: 5528/5976`
   (мешинг работает, геометрия реально намешана) — игрок был на координатах вида
   `(2000, 74, 200000)`. Это оказалось новым, отдельным багом.

**Причина (2)**: `EVInstance.renderFarLod` строил `mvp = proj * poseStack.pose()` и передавал
эту матрицу и в `extractFrustumPlanes`, и в шейдер `far_lod_cube.vert`, но
`sectionBoundingSphere(SectionPos)` и вершины куба в шейдере (`node.bounds.xyz`) работали в
**абсолютных мировых координатах** (`pos.minBlockX()` и т.п.). `RenderLevelStageEvent#getPoseStack()`
в NeoForge — camera-relative по контракту движка (подтверждено официальной рекомендацией
NeoForge "Always account for camera position when rendering. Subtract camera position from world
coordinates", а также документацией Iris по terrain-uniforms: сама позиция камеры уже вычтена
до того, как PoseStack передаётся мод-коду — стандартная практика Minecraft с 1.14+ для защиты
от float-precision проблем вдали от origin). Рассинхрон систем координат (world-absolute vs
camera-relative) на малых координатах почти незаметен (обе системы почти совпадают у origin), но
на `Z ~ 200000` расхождение — десятки тысяч блоков: все bounding sphere проваливали frustum-тест
(`visible-sections: 0`), а те немногие, что всё же проходили бы, транслировались бы шейдером в
координаты вне экрана/с грубыми артефактами.

**Фикс** (`EVInstance.renderFarLod`, `ev-neoforge/.../EVInstance.java`):
- `mvp` (`proj * poseStack.pose()`) остаётся построенным как раньше — подтверждено, что
  `poseStack.pose()` уже rotation-only/camera-relative, так что этот `mvp` корректен для
  трансформации именно **camera-relative** точек (мировая координата минус позиция камеры).
- Новая локальная функция `cameraRelativeBounds` (`Function<SectionPos, float[]>`) — обёртка
  вокруг существующего `sectionBoundingSphere`, вычитающая `cameraX/Y/Z` из центра сферы перед
  возвратом. Поворот не применяется здесь намеренно — он уже заложен в `mvp` через
  `poseStack.pose()`, так что остаётся только вычесть трансляцию.
- `traversal.computeVisible(nearbySections, frustum, cameraRelativeBounds)` — раньше передавался
  `this::sectionBoundingSphere` (абсолютные координаты), теперь `cameraRelativeBounds`.
- `FarLodPassRenderer.record(commands, visibleSections, cameraRelativeBounds, viewProjColumnMajor)` —
  аналогично, шейдер теперь получает координаты уже в той же системе, что и `uViewProj`.
- `filterByDistance` (шаг 5, дистанционный cutoff) **не изменён** — он сравнивает абсолютные
  `sectionBoundingSphere` против абсолютных `cameraX/Y/Z`, и эта разность инвариантна к общему
  сдвигу системы координат, так что был и остаётся корректным независимо от абсолютной величины
  координат.

**Почему не тронут `filterByDistance`, но тронут только frustum/шейдер**: единственная операция,
чувствительная к coordinate-space mismatch — это сравнение мировых координат с матрицей, которая
имплицитно предполагает camera-relative вход (frustum planes из `mvp`, и `gl_Position = uViewProj
* vec4(worldPos, 1.0)` в шейдере). Вычитание дистанции (`dx = bounds[0] - cameraX` и т.п.) не
зависит от того, в какой системе координат находятся обе стороны вычитания, пока обе — в одной
и той же (обе абсолютные) — поэтому этот код был и остаётся корректным без изменений.

**Тесты** (`EVInstanceTest.java`): фикс дополнительно вынесен как отдельная чистая
static-функция `EVInstance.toCameraRelative(float[] worldBounds, cameraX, cameraY, cameraZ)`
(вычитает камеру из центра сферы, радиус не трогает) — тот же паттерн вынесения в
package-private static ради юнит-тестируемости без GL/`Minecraft`, что уже применялся для
`filterByDistance`/`calculateNearCutoffBlocks`. `renderFarLod`'s `cameraRelativeBounds` теперь
просто вызывает `toCameraRelative(sectionBoundingSphere(pos), camX, camY, camZ)` вместо
инлайновой лямбды. 3 новых сценария: базовое вычитание камеры (радиус не меняется), no-op на
камере в origin (регрессионная защита — гарантирует точное совпадение с поведением до фикса
вблизи (0,0,0)), и прямое воспроизведение чисел из репорта бага (`(2016,90,200016)` секция при
камере на `(2000,74,200000)` → относительные координаты `(16,16,16)`, малые числа вместо
абсолютных ~200000).


**Не подтверждено реальным Gradle-билдом или запуском в игре в этой сессии** — песочница
по-прежнему без сетевого доступа к репозиториям Minecraft/NeoForge и без установленного `javac`
(`./gradlew` падает: `Could not find or load main class org.gradle.wrapper.GradleWrapperMain`,
сам wrapper jar недоступен офлайн). Проверено вручную: баланс скобок обоих изменённых файлов,
типовая совместимость сигнатур (`Function<SectionPos, float[]>` совпадает в
`traversal.computeVisible` и `FarLodPassRenderer.record`), логическая согласованность систем
координат по каждому шагу, а также сверены существующие тесты `EVInstanceTest` — они бьют
только в `filterByDistance`/`calculateNearCutoffBlocks` (не тронуты этим фиксом), так что
регрессий по уже существующим сценариям быть не должно.

Требует от пользователя пересборки (`./gradlew :ev-neoforge:build`) и повторения того же
сценария (телепорт на большие координаты вроде `(2000, 74, 200000)`, снятие `/ev debug` снапшота)
для визуального подтверждения, что `visible-sections` перестал быть нулём и геометрия
действительно рисуется на экране в правильном месте.

**Отдельно, не в scope этого фикса**: сам факт, что `sectionBoundingSphere` продолжает
возвращать абсолютные мировые координаты (используется также в `filterByDistance` и, вероятно,
пригодится будущим тикетам вроде HiZ-occlusion/indirect multidraw из Волны 2) — оставлен как
есть намеренно, чтобы не расширять эту сессию за пределы найденного бага; camera-relative
конвертация сделана точечно только там, где она реально нужна (frustum test + GPU upload).
Будущим тикетам, добавляющим новые точки использования `sectionBoundingSphere` для чего-либо,
что взаимодействует с view/projection матрицами, стоит явно проверять, нужна ли им
camera-relative версия — эта путаница абсолютных/относительных координат легко
воспроизводится заново, если не держать её в уме как системное правило проекта.

## Сессия: геометрия "приклеена к экрану" (следует за pitch/yaw камеры) после camera-relative фикса

**Контекст**: пользователь подтвердил, что тестировал уже пересобранный билд с предыдущим
фиксом (camera-relative bounding spheres), и предоставил 3 скриншота + `latest.log` +
`debug.log` + `ev-coldstart.jfr`. На скриншотах видны крупные полупрозрачные цветные грани
(far-LOD debug-кубы из `FarLodPassRenderer`, цвет по `gl_InstanceID % 8`), которые корректно
проходят depth-test относительно реального террейна (окклюзия работает в обе стороны — иногда
деревья видны поверх куба, иногда куб перекрывает небо), но геометрически ведут себя неправильно:
по словам пользователя, при изменении pitch (наклон взгляда вверх/вниз) вся группа кубов
синхронно "едет" вместе со взглядом, как будто прицеплена к экрану, а не к миру.

**Диагностика**: этот симптом — устойчивый паттерн "MVP без реального поворота камеры": если
итоговая `view`-часть MVP не содержит поворота (например, единичная матрица вместо реальной
view-матрицы), то `mvp = proj * identity = proj` — чистая перспективная проекция без учёта
направления взгляда. Геометрия с фиксированными (пусть даже некорректными по позиции)
camera-relative координатами будет в таком случае визуально "прилипать" к экрану и двигаться
вместе с любым поворотом камеры 1-в-1, что и было визуально описано и видно на скриншотах
(куб остаётся в кадре даже при взгляде прямо в зенит, кадр 3).

**Вероятная причина**: предыдущий фикс использовал `event.getPoseStack().last().pose()` как
источник camera-rotation матрицы. `RenderLevelStageEvent` в NeoForge экспонирует ДВА разных,
независимо переданных в конструктор значения — `poseStack` и `modelViewMatrix` (см. javadoc
`RenderLevelStageEvent(LevelRenderer, LevelRenderState, @Nullable PoseStack poseStack, Matrix4f
modelViewMatrix, ...)` и более старую сигнатуру для нашей версии 1.21.1/NeoForge 21.1.x с
дополнительным отдельным `projectionMatrix`). `getPoseStack()` компилируется и не равен `null`
на стадии `AFTER_SOLID_BLOCKS`, но нет гарантии, что `poseStack.last().pose()` — это именно та
матрица, с которой рендерится ванильный террейн; `modelViewMatrix`, судя по официальному
описанию API ("Returns the model view matrix used for rendering") и DeepWiki-обзору рендер-пайплайна
NeoForge ("ModelViewMatrix - The current model-view transformation matrix"), — специально
предназначенное для этого значение.

**Фикс** (`EVInstance.renderFarLod`): заменён источник view-матрицы —
`event.getPoseStack().last().pose()` → `event.getModelViewMatrix()`. `mvp = proj *
modelViewMatrix` и вся остальная camera-relative логика (`cameraRelativeBounds`,
`toCameraRelative`) не менялись — само направление фикса (camera-relative трансляция) остаётся
верным, менялся только источник матрицы поворота.

**Уровень уверенности и что НЕ подтверждено**: я не смог найти текстовое подтверждение точной
формулы построения `modelViewMatrix` внутри патча `LevelRenderer.java.patch` (сам патч не
проиндексирован публичным текстовым поиском построчно) — только официальное описание метода
("model view matrix used for rendering") и общий факт, что вместе с ним террейн использует
отдельный per-chunk world-space offset (аналогично `ChunkOffset` в терминах Iris/шейдерных модов).
Это достаточно веское основание, что `modelViewMatrix` — правильный, camera-rotation-содержащий
источник (в отличие от `poseStack`, который не гарантированно им является на этой стадии), но
это не то же самое, что построчное чтение патча. **Требует подтверждения пользователем** на
живом билде: пересобрать, повторить тот же сценарий (наклон камеры по pitch с видимыми far-LOD
кубами) и проверить, что геометрия перестала "ехать" за взглядом и вместо этого либо остаётся
на месте в мире, либо корректно проходит/не проходит frustum-culling в зависимости от реального
направления взгляда.

**Не подтверждено реальным Gradle-билдом в этой сессии** — та же причина, что и раньше (нет
